...
Due to a reasonable RSME value considering the data size, our model is a good model. The error in the predicted height is only about plus/minus 2.24 inches which is not a significantly large error. 
...
It is reasonable that parents and their children have the same height because childern inherit the parents DNA which play a role on the childs height.
...
I was shortened to "regression to the mean" beause Galton observed that after an extreme result in the height of the first child then the height of next the child will then be closer to the mean, thus regressing to the mean.

Why should this effect be real?

This effect should be real because through Galton's findings, it was shown how the heights of the childern of the same parents got closer to the mean after the first. 
