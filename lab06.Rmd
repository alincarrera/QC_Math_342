...
```{r}
in_sample_s_e = sd(y - y_train)
oos_s_e = sd(y - y_test)

in_sample_s_e
oos_s_e
#The out of sample error is greater
```
...
for (i in 1 : 1000){
  ...
  in_sample_s_ej = in_sample_s_ej + sd(y - y_train)
  oos_s_ej = oos_s_ej + sd(y - y_test)
}

abs((in_sample_s_ej - oos_s_ej)/1000)
```
...
for (i in 1 : n){
  ...
  in_sample_s_e = lm(y_train ~ ., data.frame(Xjunk_train))
  se_s = sd(in_sample_s_e$residuals)
}
...
```{r}
X2dummyfeatures = as.matrix(X2dummy[, 2])
```
...
```{r}
Xdummy = array(Xjdummyfeatures)
 for (j in 2 : 13){
  xj = X[, j]
  xj_bin = cut(xj, breaks = unique(quantile(xj, c(0, .5, 1))), include.lowest=TRUE)
  Xjdummy = model.matrix(~ 0 + ., data.frame(xj_bin))
  Xjdummyfeatures = as.matrix(Xjdummy[, j])
}
```
...
```{r}
X_dummies = cut(Xdummy, breaks = unique(quantile(xj, c(.33, .66, 1))), include.lowest=TRUE)
```
