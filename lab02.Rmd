...
```{r}
v = sample(rnorm(1000, mean = -10, sd = sqrt(100)))
v
```
...
flip_matrix = function(X, dim_to_rev){
  if(dim_to_rev == 2){
      return(X[nrow(X):1, ])
    }
    else {
...
# The estimate is close to what is expected by theory.
```
...
# Using qnorm we get that it should be -inf theortically, but our estimate gives us number that not close to what is expected by theory. 
```
...
```{r}
n = 16777216
my_list = list("A" = array(1 : n, c(1)), 
               "B" = array(1 : n, c(2, 2)), 
               "C" = array(1 : n, c(3, 3, 3)), 
               "D" = array(1 : n, c(4, 4, 4, 4)), 
               "E" = array(1 : n, c(5, 5, 5, 5, 5)), 
               "F" = array(1 : n, c(6, 6, 6, 6, 6, 6)), 
               "G" = array(1 : n, c(7, 7, 7, 7, 7, 7, 7)), 
               "H" = array(1 : n, c(8, 8, 8, 8, 8, 8, 8, 8)))
my_list
```
...
The object.size function gives an estimation of memory that is used to store objects, looking at my_list it makes sense that a lot more memory is being used to store the arrays with larger dimensions.
