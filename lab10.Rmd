...
```{r}
test_prop = 0.1
train_indices = sample(1 : nrow(bills_data), round((1 - test_prop) * nrow(bills_data)))
...
test_indices = setdiff(1 : nrow(bills_data), train_indices)
...
nrow(bills_data_train) / get_tree_num_nodes_leaves_max_depths(tree_mod)$num_leaves
...
x_trains = matrix(NA, nrow = Nsim, ncol = n)
y_trains = matrix(NA, nrow = Nsim, ncol = n)
all_oos_residuals = matrix(NA, nrow = Nsim, ncol = n)
for (nsim in 1 : Nsim){
  #simulate dataset $\mathbb{D}$
  x_train = runif(n, xmin, xmax)
  delta_train = rnorm(n, 0, sigma) #Assumption I: mean zero and Assumption II: homoskedastic
  ...
  x_test = runif(n, xmin, xmax)
  delta_test = rnorm(n, 0, sigma)
...
cat("oos standard error reduction:", (1 - s_e_bag / s_e) * 100, "%\n")
```

What is your gain over the unbagged model? Why is there a gain?

The gain was about 95%.
